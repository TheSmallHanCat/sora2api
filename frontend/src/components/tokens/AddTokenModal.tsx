...
export const AddTokenModal: React.FC<AddTokenModalProps> = ({
  ...
  const resetForm = () => {
    ...
    setImageConcurrency('-1');
    setVideoConcurrency('-1');
  ...
  const handleSubmit = async () => {
    if (!at.trim()) {
      showToast('Введите Access Token или используйте конвертацию ST/RT', 'error');
      return;
    }
    ...
    try {
      ...
      if (result.status === 409) {
        const msg = result.detail || 'Token уже существует';
        showToast(msg, 'error');
        return;
      }
      ...
      if (result.success) {
        ...
        showToast(`Ошибка добавления: ${result.detail || result.message || 'Неизвестная ошибка'}`, 'error');
      }
    } catch (err) {
      showToast(`Ошибка добавления: ${err instanceof Error ? err.message : 'Неизвестная ошибка'}`, 'error');
    } finally {
      setIsLoading(false);
    }
  };
